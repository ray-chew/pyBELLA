                         ...
                         const double dt)
{
    ...
    if (ud.mol_trans == FULL_MOLECULAR_TRANSPORT) {
        ...
        molecular_fluxes(flux, Sol, elem);
            ...
            case 2: {
                ...
                for (int j=igy-1; j<icy-igy+1; j++) {
                    ...
                    for (int i=igx-1; i<icx-igx+1; i++) {
                        ...
                        diss[ncij]       = - Sol->rho[ncij]*( (flux[0]->rhoY[nfxij+1] - flux[0]->rhoY[nfxij])/dx + (flux[1]->rhoY[nfyij+1] - flux[1]->rhoY[nfyij])/dy);
