...
constant specific heat capacities may be written as 
%
...
\end{equation}
%
are the Exner pressure and the density weighted potential temperature, $\Theta$, respectively, 
with $\rfr{p}$ as suitable reference pressure, $R$ the gas constant and $c_p$ and $c_v$ the 
...
the ``$\beta$-plane'', and $\vk$ the vertical unit vector. 
...
equation for $\chi$ and splitting 
%
\begin{equation}
...
to obtain
%
\begin{subequations}\label{eq:PsincII}
...
\dss (\rho w)_t 
  & + 
    & \dss \nabla_\parallel\cdot(P \vu\, \chi w) + (Pw\, \chi w)_z \hfil
      & = 
        & \dss - \left(\frac{P}{\Gamma} \pi_z + \rho g\right)
          \\[15pt]
\label{eq:Peqn}
P_t
  &  +
...
%
\begin{equation}
\Psi = (\chitilde, \chi\vu, \chi w, 1)
\end{equation}
% 
by the advecting fluxes 
%
\begin{equation}
P\vv \equiv (P\vu, Pw)\,,
\end{equation}
% 
where by ``advection of 1'' we refer to solving \eq{eq:Peqn}. The right hand 
side column in \eq{eq:PsincII} collects all non-advective forcings. In compact 
notation \eq{eq:PsincII} reads as
%
\begin{equation}
(P\Psi)_t + \mathcal{A}(\Psi; P\vv) = R(P\Psi)\, ,
\end{equation}
%
and with this notation an EULAG-type time integration scheme for the update
of the solution from time $t^n$ to time $t^{n+1}$ reads 
%
...
\dss (P\Psi)^{*} 
  & = 
    & \dss (P\Psi)^{n} + \frac{\Delta t}{2} R\left((P\Psi)^n\right)
      \\
\label{eq:EULAGAdvectionStep}
\dss (P\Psi)^{**} 
  & = 
    & \dss \mathcal{A}_{2\text{nd}}^{\Delta t}\left((P\Psi)^*; (P\vv)^{n+1/2}\right)
      \\
\label{eq:EULAGImplicitEulerStep}
\dss (P\Psi)^{n+1} 
  & = 
    & \dss (P\Psi)^{**} + \frac{\Delta t}{2} R\left((P\Psi)^{n+1}\right)\,.
\end{IEEEeqnarray}
\end{subequations}
%
Here $\mathcal{A}_{2\text{nd}}^{\Delta t}\left(\Psi; \vV\right)$ denotes a
second-order accurate explicit approximation of the \emph{linear} 
advection of $\Psi$ by the \emph{given} advecting fluxes $\vV$ 
...
\eq{eq:EULAGAdvectionStep} are at least first order accurate approximations 
of the $P\vv$ field at the half time level $t^{n+1/2} = (t^n + t^{n+1})/2$ 
that can be determined in various ways. The default option in EULAG is 
extrapolation from the last two completed time levels at $t^{n-1}$ and
$t^{n}$, which makes the method a three time level scheme. The option adopted 
here and in alternative truly two time level variants of EULAG is to carry out 
an intermediate, at least first order accurate, time step to the half time 
level to evolve $(P\vv)^{n}$ to $(P\vv)^{n+\half}$ without a need to recur to 
...
$D\Psi/Dt \equiv (\partial_t + \vv\cdot\nabla) \Psi = R(P\Psi)/P$ 
that holds along Lagrangian characteristics of \eq{eq:PsincII}.  A 
...
An observation that may in part explain the excellent stability features of 
EULAG, which are essentially inherited by the scheme proposed here, is that 
for given advecting flux $\vV = (P\vv)^{n+\half}$ the advection step is 
\emph{linear}. As a consequence, even if for rather large time steps the 
velocities computed in the forward Euler step \eq{eq:EULAGExplicitEulerStep} 
end up far away from the solutions at time levels $n$ or $n+1$ for large
...
scheme. Thus it is legitimate to use the following simplified first order 
splitting version of \eq{eq:EULAGTimeIntegrator} for this purpose,
%
\begin{subequations}\label{eq:EULAGHalfTimePredictor}
\begin{IEEEeqnarray}{rCl}
...
\dss (P\Psi)^{\#} 
  & = 
    & \dss \mathcal{A}_{1\text{st}}^{\frac{\Delta t}{2}}\left(\Psi^{n}; (P\vv)^{n}\right)
      \\
\label{eq:EULAGHalfTimePredictorRHS}
\dss (P\Psi)^{n+1/2} 
  & = 
    & \dss (P\Psi)^{\#} + \frac{\Delta t}{2} R\left((P\Psi)^{n+1/2}\right)\,,
\end{IEEEeqnarray}
\end{subequations}
%
where $\mathcal{A}_{1\text{st}}^{\Delta t}$ denotes a first order
...
the final implicit half time steps in \eq{eq:EULAGImplicitEulerStep}
and \eq{eq:EULAGHalfTimePredictorRHS}, which are conceptually identical, 
except that \eq{eq:EULAGImplicitEulerStep} leads to divergence-controlled
cell-centered velocities, whereas \eq{eq:EULAGHalfTimePredictorRHS} generates
divergence-controlled advective fluxes at the cell faces that comply with 
the pressure equation \eq{eq:Peqn}.
...
algorithm'' (MPDATA) \cite{PrusaEtAl2008}. The present implementation involves
...
\end{equation}
%
and an analogous formula holds for $(Pv)^{n+\half}_{i,j+\half}$. Given these
fluxes, the advection step in \eq{eq:EULAGAdvectionStep} is discretized via
Strang splitting, so that
%
...
with 
%
\begin{subequations}\label{eq:AdvSpecifics}
\begin{IEEEeqnarray}{rCl}
\Psi_{i+\half,j} 
  & = 
...
\Psi^{+}_{i+\half,j} 
  & = 
    & \Psi_{i+1,j} - \frac{\Delta x}{2} \left(1 + C^{n+\half}_{i+\half,j} \right)  s_{i+1,j}\,,
      \\[8pt]
\label{eq:AdvSpecificsCourantNo}
C^{n+\half}_{i+\half,j}
  & =
...
\cite{Sweby1984}). The effect of the slope limiting procedure on our simulation
...
reduction to first order accuracy in the predictor step not only does not
affect the formal order of consistency of the scheme, but also hardly 
influences the solution quality at finite resolution.

% ===========================================================================
% ===========================================================================

\subsection{Explicit Euler step for the generalized source terms}
...
\subsection{(Linearized) implicit Euler step for the generalized source terms}
