...
unknowns $(\rho, \rho\vu, \rho w, P)$ at the new time level. \benacchio{two paragraphs above was an unknown, now it is an auxiliary variable? Perhaps this ambiguity is what your comment below aims at.} \klein{[That
...
all subsequent simulations we set $\chibar_k^{n} \equiv \chibar_k^{0}$. \benacchio{Is this not like using a constant reference state as everybody does? Where is the difference then?} 
...
the linearized equation for $\pi'$ would be lost.} \benacchio{Right, I think something else might be going on in the UM as well, like a monotonization of the previous time step potential temperature background distribution. I should be able to find out the details.} We leave testing 
...
as described in a similar context by \citet{SmolarkiewiczEtAl2014} \benacchio{Are these starred variables the same as in \eqref{eq:ExplicitEulerStep} or a general base state?}. The 
...
%
  \begin{enumerate}\item[]
    ...
    \item[(iii)] Use the advective fluxes to obtain $P^{n+1}$ through \eq{eq:PUpdate}. (\benacchio{so if this is already at the new time level, I don't think \eq{eq:AdvectionStep} is used for $P$, also because that variable is not part of $\Psi$...})
...
\item[\emph{Auxiliary variables update}]
  ...
  \begin{enumerate}\item[]
    \item[(i)] The auxiliary variables $(\chi', \piprime)$ are computed from the primary unknowns at the new time level. \benacchio{But was $\chi'$ not part of $\Psi$? Then this step is already included in part (i) of the previous stage...}
  \end{enumerate}
...
\item differences to Hilary's scheme \cite{weller:2014}

\item differences to ENDGAME, GUNGHO and the like \cite{wood:2013} \cite{melvin:2018}

\item differences to George Bryan's model \cite {bryan:2002}

\item differences to COSMO, ICON etc. ... \cite{baldauf:2011, zaengl:2015}
