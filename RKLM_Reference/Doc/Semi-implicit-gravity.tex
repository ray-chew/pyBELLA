...
\end{IEEEeqnarray}
%
In the semi-implicit discretization we will carry $\chitilde$ as an
...
forced advection equations in the ``EULAG-approach'' described in section~\ref{ssec:EULAG}, 
see \eq{eq:RhoBasedAdvection}, we split the augmented equations from \eq{eq:EulerP} and 
\eq{eq:EulerPChiTilde} into the internal energy equation, \eq{eq:EulerPP}, and a set of 
forced advection equations
%
...
$P$-$\pi$ coupling through the equations of state as currently done in the code
is correctly captured by the notation in \eq{eq:ImplicitEulerStep}.
%
...
$\chi = \chitilde + \chibar$ according to \eq{eq:ChiSplit} and the subsequent 
separate updates of  $\chi$ and $\chitilde$ governed by discrete versions
of the full and perturbed mass balances in \eq{eq:chiI} and 
\eq{eq:EulerPChiTilde}, respectively, require re-synchronization. Thus, 
after completion of a time step we let
%
\begin{equation}
\chitilde_{i,j}^{n} = \chi_{i,j}^{n} - \chibar_{j}^{n}\,,
\end{equation}   
%
...
auxiliary potential temperature perturbation equation \eq{eq:EulerPChiTilde} 
may be written as
%
...
%
\begin{equation}
(U,V,W,\Xtilde) = (P u, P v, P w, P\chitilde)\,,
\end{equation}
%
