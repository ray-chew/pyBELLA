...
\end{equation}
%
and where $\mathcal{A}(\Phi; \rho\vv)$ and $R(\rho, \Phi)$ denote the advection terms 
on the left and the right hand side terms of 
\eq{eq:EULAGHorMomEqn}--\eq{eq:EULAGThetaTildeEqn}, respectively. 
...
a robust second-order accurate explicit approximation of the \emph{linear} 
advection\footnote{MPDATA: Multidimensional Positive Definite Advection Transport 
Algorithm} of $\Phi$ by the \emph{given} advecting fluxes $\vV$ 
...
to obtain
%
\begin{IEEEeqnarray}{rCrCl}\label{eq:PsincII}
\dss \rho_t 
  & + 
    & \dss \nabla_\parallel\cdot(P\vu\, \chi) + (Pw\, \chi)_z \hfil
      & = 
        & \dss 0
          \IEEEyesnumber\IEEEyessubnumber*\label{eq:RhoEqn}\\[5pt]
\dss (\rho\vu)_t 
  & + 
...
\end{IEEEeqnarray}
%
where \eq{eq:ChiTildeEqn} is redundant relative to \eq{eq:RhoEqn} but turns out
to be useful as an auxiliary equation in the construction of a semi-implicit 
discretization of buoyancy effects.

The second column of terms on the left represents advection of 
%
\begin{equation}
\Psi = (\chi, \chi\vu, \chi w, 1, \chitilde)
\end{equation}
% 
...
\end{equation}
% 
where by ``advection of 1'' we refer to solving \eq{eq:Peqn} for \emph{given}
advective fluxes $P\vv$. The right hand 
...
%
\begin{IEEEeqnarray}{rCl}\label{eq:EULAGHalfTimePredictor}
\dss (P\Psi)^{\#} 
  & = 
    & \dss \mathcal{A}_{1\text{st}}^{\frac{\Delta t}{2}}\left(\Psi^{n}; (P\vv)^{n}\right)
      \IEEEyesnumber\IEEEyessubnumber*\label{eq:EULAGHalfTimePredictorAdvection}\\
\dss (P\Psi)^{n+1/2} 
  & = 
...
There is one notable conceptual difference between the present approach for 
compressible flow and the ``consistent framework for discrete integrations 
of soundproof and compressible {P}{D}{E}s of atmospheric dynamics'' by 
\citet{SmolarkiewiczEtAl2014}, which also builds on the general 
forward-in-time integration strategy reviewed by \citet{PrusaEtAl2008} 
...
\subsection{Pressure synchronization}
\label{ssec:PressureSynchronization}

The pressure $p$ plays a multiple roles in the compressible Euler equations
as formulated in \eq{eq:PsincI}. The cell-centered volume average of 
$P = (\rfr{p}/R) (p/\rfr{p})^{c_v/c_p}$ is representative of the energy
...
the implicit Euler steps in \eq{eq:EULAGImplicitEulerStep} and 
\eq{eq:EULAGHalfTimePredictorRHS}, respectively, we will need both 
