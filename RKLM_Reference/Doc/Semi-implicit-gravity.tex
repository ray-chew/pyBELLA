...
time stepping algorithm, see \eq{eq:TimeIntegrator} below. We note in particular
...
\begin{figure}
\centering
 \red{Sketch of grid arrangement}
\caption{Sketch of grid arrangement \label{fig:GridArrangement}}
\end{figure}
...
hold for $\mathcal{A}^{\Delta t/2}_y U_{i,j}$. 
...
\subsection{Implicit Euler step for the forcing terms}
\label{ssec:ImplicitEulerForcing}

Since both $\rho$ and $P$ are frozen in this split step, the relevant 
linearized equations may be written as
%
...
\end{equation}
%
and where the coefficient $(P\Theta)^*$ is time independent and prescribed. The 
...
U^{n+1}
  & = 
    & U^{n} - \Delta t \left( c_p (P\Theta)^* {\pi'}^{n+1}_x - f V^{n+1}\right)
      \IEEEyesnumber\IEEEyessubnumber*\\[7pt]
V^{n+1}
  & = 
    & V^n - \Delta t \left( c_p (P\Theta)^* {\pi'}^{n+1}_y + f U^{n+1} \right)
      \\[0pt]
W^{n+1}
  & =
    &  W^n - \Delta t 
...
{\Thetatilde}^{n+1}
  & =
    & {\Thetatilde}^{n} - \Delta t\frac{d\Thetabar}{dz}\, W^{n+1}
      \\
\left(\frac{\partial P}{\partial \pi}\right)^* {\pi'}^{n+1}
  & =
    & \left(\frac{\partial P}{\partial \pi}\right)^* {\pi'}^{n} 
    - \Delta t \left( U^{n+1}_x + V^{n+1}_y + W^{n+1}_z \right)\,.
...
%      \right)
    ...
    - \Delta t \, c_p (P\Theta)^* 
      \left(
        \begin{array}{c}
        \pi'_x + \Delta t\, f\, \pi'_y \\ 
        \pi'_y - \Delta t\, f\, \pi'_x
        \end{array}
      \right)^{n+1}
      \right]
      \IEEEyesnumber\IEEEyessubnumber*\\
W^{n+1} 
  & =
    & \frac{W^n + \Delta t\, g {\Thetatilde}^{n}/\Thetabar}{1 + (\Delta t\, N)^2}
      - \Delta t \, \frac{c_p (P\Theta)^*}{1 + (\Delta t\, N)^2} \ {\pi'}^{n+1}_z
\end{IEEEeqnarray}
%
where
%
...
solution, backward re-insertion yiels $(U, V, W, \Thetatilde)^{n+1}$. 

% ===========================================================================
% ===========================================================================

\subsection{Divergence controlled advective fluxes via 
\eq{eq:HalfTimePredictorFluxCorrection}}
\label{ssec:DivControlledAdvectiveFluxes}

The goal of solving \eq{eq:HalfTimePredictorFluxCorrection} is to obtain first
...
(Pu)^{\#}_{i+\half,j} 
  & = 
    & (\rho u\chi)^{\#}_{i,j} + (\rho u\chi)^{\#}_{i+1,j}
      \IEEEyesnumber\IEEEyessubnumber*\\
(Pw)^{\#}_{i,j+\half} 
  & = 
    & (\rho w\chi)^{\#}_{i,j} + (\rho w\chi)^{\#}_{i,j+1}
\end{IEEEeqnarray}
%
of the advective fluxes. These differ from the corresponding data at time
level $t^n$ only through the effect of the advection step, so that a consistent
...
Specifically, we include these effects through a linearized implicit Euler
step. 
...
(Pu)^{n+\half}_{i+\half,j} 
  & = 
    & (Pu)^{\#}_{i+\half,j} 
      - \frac{\Delta t}{2}
        \left( c_p\left(\frac{P}{\chi}\right)^{\#}_{i+\half,j}
               \frac{\pi^{n+\half}_{i+1,j} - \pi^{n+\half}_{i,j}}{\Delta x}
      + f_0 (Pv)^{n+\half}_{i+\half,j}
    \right)
      \IEEEyesnumber\IEEEyessubnumber*\\
(Pv)^{n+\half}_{i+\half,j} 
  & = 
    & (Pv)^{\#}_{i+\half,j} 
      - \frac{\Delta t}{2} f_0 (Pu)^{n+\half}_{i+\half,j}
      \\
(Pw)^{n+\half}_{i,j+\half} 
  & = 
    & (Pw)^{\#}_{i,j+\half} 
      - \frac{\Delta t}{2}
        \left( c_p\left(\frac{P}{\chi}\right)^{\#}_{i,j+\half}
               \frac{\pi^{n+\half}_{i,j+1} - \pi^{n+\half}_{i,j}}{\Delta z}
      + g P^{n+\half}_{i,j+\half}
    \right)
\end{IEEEeqnarray}
%
