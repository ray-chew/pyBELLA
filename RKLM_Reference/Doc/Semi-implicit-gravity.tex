...
structural model relationship, they introduced the inverse of the potential 
temperature,
%
\begin{equation}
\chi = 1/\Theta\,,
...
In formulating a scheme that is implicit with respect to pressure as well as
buoyancy and potential temperature advection, it is crucial to have separate 
access to the large-scale mean background stratifications of pressure and 
potential temperature, or of its inverse, and to their local perturbations. 
...
\end{equation}
% 
where the background variable satisfy the hydrostatic relation
%
...
Since $P$ can be expressed as a function of $\pi$ alone and as $\pibar$ is
time independent across a time step, the perturbation Exner pressure satisfies
%
...
Auxiliary discretizations of equations \eq{eq:EulerPiPrime} and 
...
the overal structure when reading the more detailed stuff further down. 
...
$t^{n+\half} = (t^{n} + t^{n+1})/2$. This is achieved by applying standard splitting
into advective and non-advective terms in \eq{eq:EulerP}, \eq{eq:EulerPChiPrime}. 
...
of an implicit Euler discretization of the full system (see 
...
\end{IEEEeqnarray}
%
With the linearization, this implicit step involves a single linear elliptic solve
for $\pi^{n+\half}$, while without the linearization an outer iteration 
can be invoked to guarantee consistency with the equation of state up to the 
desired accuracy. 
...
time stepping algorithm, see \eq{eq:AdvectionStep} below. We note in particular
that for given advective fluxes the update of the internal energy variable,
$P$ is determined, i.e., 
%
\begin{equation}
...
\end{equation}
%
is known and the remainder of the time stepping procedure serves to update
the vector of specific variables $\Psi$ from \eq{eq:PsiDefinition}. 
...
semi-implicit time step of the proposed scheme reads
%
\begin{IEEEeqnarray}{rCl}\label{eq:TimeIntegrator}
\dss (P\Psi)^{*} 
  & = 
    & \dss (P\Psi)^{n} + \frac{\dt}{2} R\left(\Psi^n; P^n\right)
...
\dss (P\Psi)^{n+1} 
  & = 
    & \dss (P\Psi)^{**} + \frac{\dt}{2} R\left(\Psi^{n+1}; P^{n+1}\right)\,.
...
entirely determined by the advection step in \eq{eq:AdvectionStep}. Therefore,
the unknowns that effectively get updated in the explicit and implicit Euler 
steps are $(\vu, w, \chiprime)$ only. Nevertheless, to obtain an appropriate 
approximation of the Exner pressure field needed in the momentum equation, 
an auxiliary, redundant discretization of the energy equation 
\eq{eq:EulerPP} is utilized as described in more detail in 
section~\ref{ssec:SemiImplicitForcing} below. 

After completion of the steps in \eq{eq:TimeIntegrator} the values of the
auxiliary variables $(\chiprime, \pi')$ are re-computed from the primary
unknowns $(\rho, \rho\vu, \rho w, P)$ at the new time level.
...
\caption{Cartesian grid arrangement. $C$: primary finite volumes, 
...
$\dx, \dy$, and grid indices $i = 0, ..., I-1$ and 
$j = 0, ..., J-1$ (see Fig.~\ref{fig:GridArrangement}). 
The primary variables of the discrete approximate 
solution consists of averages of grid cell averages 
%
\begin{equation}
\Sol_{i,j}^n \approx 
\frac{1}{\dx \dy}\int\limits_{C_{i,j}} \Sol(\vx,t^n)\, d^2\vx
\qquad\text{of}\qquad 
\Sol = \left(
\begin{array}{c}
\rho \\ \rho\vu \\ \rho w \\ P
...
%
\begin{equation}
(Pu \Psi)^{n+\half}_{i+\half,j} 
\qquad\text{and}\qquad
(Pv \Psi)^{n+\half}_{i,j+\half} 
\end{equation}
%
...
Similarly, $(Pu)^{n+\half}_{i+\half,j}$ and $(Pv)^{n+\half}_{i,j+\half}$ 
are staggered-grid components of the advective fluxes $(P\vv)^{n+\half}$.
...
based fluxes $(Pu)^{n+\half}_{i+\half,j}$ and $(Pv)^{n+\half}_{i,j+\half}$
and naturally works with cell- and timestep-centered values of the Exner 
pressure. The latter corrects the cell-centered momenta and requires 
integrals of the Exner pressure over grid cell faces, which we compute 
from node-centered values by the trapezoidal rule. Thus we compute both
%
\begin{equation}\label{eq:ExnerPressures}
\pi_{i,j}^{n+\half} 
\qquad\text{and}\qquad
\pi_{i+\half,j+\half}^{n+1} 
\end{equation}
%
...
The pressure $p$ plays multiple roles in the compressible Euler equations
...
the pressure. As a consequence, in controlling both the advective fluxes 
$(Pu)_{i+\half,j}, (Pv)_{i,j+\half}$ at cell faces and the momenta 
$(\rho \vv)_{i,j}$ at the cell centers through 
the implicit Euler steps in \eq{eq:HalfTimePredictorFluxCorrection} and 
\eq{eq:TimeIntegrator}, respectively, we employ both node and cell centered 
approximations of the Exner pressure (see \eq{eq:ExnerPressures} and
sections~\ref{ssec:DivControlledAdvectiveFluxes} and 
\ref{ssec:ExplicitImplicitEuler} below). This is in agreement with our previous
developments in \citep{KleinTCFD2009,BenacchioEtAl2014}, and with the 
...
\pi^{n}_{i,j} 
  & =
    & \left(\frac{RP_{i,j}}{\rfr{p}}\right)^{\frac{R}{c_v}}
...
half time level nodal pressures $\pi^{n+\half}_{i+\half,j+\half}$ from
the cell-centered solution $\pi^{n+\half}_{i,j}$ of the advection flux 
...
$\chi = \chiprime + \chibar$ according to \eq{eq:ChiSplit} and the subsequent 
...
%
\begin{equation}
\chiprime_{i,j}^{n} = \chi_{i,j}^{n} - \chibar_{j}^{n}\,,
\end{equation}   
%
where we have assumed gravity to be aligned with the second coordinate
direction so that $\chibar$ depends on the $z$-coordinate only. Also, in 
all subsequent simulations we set $\chibar_j^{n} \equiv \chibar_j^{0}$. 
...
advection operator $\mathcal{A}^{\dt}$ introduced above. EULAG uses 
...
the advecting fluxes $(Pu)^{n+\half}_{i+\half,j}$ and $(Pv)^{n+\half}_{i,j+\half}$ 
...
\Sol_{i,j}^{**} 
=
\mathcal{A}_{2\text{nd}}^{\dt} \Sol_{i,j}^{*} 
\equiv
...
\Psi^{-}_{i+\half,j} 
  & = 
    & \Psi_{i,j} + \frac{\dx}{2} \left(1 - C^{n+\half}_{i+\half,j} \right)  s_{i,j}\,,
      \\[8pt]
\Psi^{+}_{i+\half,j} 
  & = 
    & \Psi_{i+1,j} - \frac{\dx}{2} \left(1 + C^{n+\half}_{i+\half,j} \right)  s_{i+1,j}\,,
      \\[8pt]
C^{n+\half}_{i+\half,j}
  & =
    & \frac{\dt}{\dx} \frac{(Pu)^{n+\half}_{i+\half,j}}{(P_{i,j} + P_{i+1,j})/2}\,,
      \label{eq:AdvSpecificsCourantNo}\\[8pt]
s_{i,j}
  & =
    & \text{Lim}
      \left(\frac{U_{i,j}-U_{i-1,j}}{\dx}, \frac{U_{i+1,j}-U_{i,j}}{\dx}\right)\,,
      \label{eq:AdvSpecificsSlopes}
\end{IEEEeqnarray}
%
where $P_{i,j}$ in \eq{eq:AdvSpecificsCourantNo} denotes the fourth component 
of $U_{i,j}$, and $\text{Lim}(a,b)$ is a slope limiting function 
...
hold for $\mathcal{A}^{\dt/2}_y U_{i,j}$. 
...
reduction to first order accuracy in the predictor step neither
affects the formal order of consistency of the scheme nor
...
The generalized forcings in \eq{eq:EulerP} are to be discretized in 
time by the implicit trapezoidal rule. This requires an explicit Euler step 
at the beginning and an implicit Euler step at the end of a time step. The 
implicit Euler step is also needed to compute the fluxes $(P\vv)^{n+\half}$ 
...
%
\begin{equation}
(U,V,W,\Xtilde) = (P u, P v, P w, P\chiprime)\,,
\end{equation}
%
...
Thus, the divergence of some vector field $\vV = (U,W)$ utilizes
%
\begin{equation}
\left(U_x\right)_{i,j} 
=
\frac{1}{\dx} \left(U_{i+\half,j} - U_{i-\half,j}\right)\,,
\quad
\left(W_z\right)_{i,j} 
=
\frac{1}{\dz} \left(W_{i,j+\half} - W_{i,j-\half}\right)\,.
\end{equation}
%

Correspondingly, \eq{eq:HalfTimePredictorFluxCorrection} determines the 
half time level advecting fluxes $(Pu)^{n+\half}_{i+\half,j}$ and 
$(Pw)^{n+\half}_{i,j+\half}$ located at the grid cell faces using a first
order accurate time integrator to lift the data from time level $t^n$ to
$t^{n+\half}$. Having completed the advection predictor, 
...
(Pu)^{\#}_{i+\half,j} 
  & = 
    & \frac{1}{2} 
      \left[(\rho u\chi)^{\#}_{i,j} + (\rho u\chi)^{\#}_{i+1,j}
      \right]\,, \quad
%      \IEEEyesnumber\IEEEyessubnumber*\\
      &
        & (Pw)^{\#}_{i,j+\half} 
          & = 
            & \frac{1}{2} 
              \left[(\rho w\chi)^{\#}_{i,j} + (\rho w\chi)^{\#}_{i,j+1}
              \right]
\end{IEEEeqnarray}
%
of the advective fluxes. These differ from the corresponding data at time
...
influence of the right hand side $Q$ of \eq{eq:ForcedPAdvection} as well. 
This is achieved through a linearized implicit Euler step as described in 
section~\ref{sssec:ImplicitEuler}. 

The components $(U,W)$ of $(P\vv)$ in this calculation are located at
the cell faces, so that an appropriate spacial discretization of the
pressure gradient is obtained from cell-centered values as
%
\begin{equation}
\left(\pi'_x\right)_{i+\half,j} 
\approx 
\frac{1}{\dx} \left(\pi'_{i,j} + \pi'_{i+1,j}\right)\,,
\quad
\left(\pi'_z\right)_{i,j+\half} 
\approx 
\frac{1}{\dz} \left(\pi'_{i,j} + \pi'_{i,j+1}\right)\,,
\end{equation}
%
and this leads to standard five and seven-point discrete Laplacians in 
two, respectively three, space dimensions. We have tested 
%
\begin{equation}\label{eq:NinePointStencil}
\left(\pi'_x\right)_{i+\half,j} 
\approx 
\frac{1}{\dx} \left({\pibar}'_{i,j} + {\pibar}'_{i+1,j}\right)\,,
\quad\text{where}\quad
{\pibar}'_{i,j} 
= \frac{3}{4}\pi'_{i,j} 
  + \frac{1}{8}\left(\pi'_{i,j+1} + \pi'_{i,j-1}\right)\,,
\end{equation}
%
and an analogous formulation for $(\pi'_z)_{i,j+\half}$ as well.
...
are based on \eq{eq:NinePointStencil}.
...
%
\begin{equation}
\left(\pi'_x\right){i,j} 
= 
\frac{1}{\dx} \left(\piprimehat_{i+\half,j} - \piprimehat_{i-\half,j}\right)\,,
\quad
\piprimehat_{i+\half,j} = \frac{1}{2}\left(\pi'_{i+\half,j+\half} + \pi'_{i+\half,j-\half}\right)\,,
\end{equation}
%
and an analogous formula for $\left(\pi'_z\right){i,j}$. The divergence
in \eq{eq:LinearizedNonAdvectiveImplicitEulerPi}, is formed on the basis 
of the cell-centered components of $\vV = (U,V,W)$, with 
%
\begin{equation}
\left(U_x\right)_{i+\half,j+\half} 
=
\frac{1}{\dx}\left(\Uhat_{i+1,j+\half} - \Uhat_{i,j+\half}\right)\,,
\quad
\Uhat_{i,j+\half} = \frac{1}{2}\left(U_{i,j} + U_{i,j+1}\right)\,.
\end{equation}
%
and analogous formulae for $\left(V_y,W_z\right)_{i+\half,j+\half}$.
This leads to a node-centered discretization of the pressure Helmholtz 
equation, with a nine- respectively 27-point stencil of the Laplacian, 
and thus yields the required update of the node-centered pressure field. 
