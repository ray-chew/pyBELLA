...
tuned seamlessly from solving the full compressible to solving the 
...
$t^n$ is sufficient for generating these half time level fluxes. To achieve
...
Clearly, from the perspective of their birds-eye views in 
...
of the discrete approximate solution consists of averages of grid cell 
averages 
%
\begin{equation}
\Sol_{i,j,k}^n \approx 
...
cell- and timestep-centered values of the Exner pressure. The latter 
...
field to the pressure. As a consequence, in controlling both the advective 
fluxes $(P\vv)^{n+\half}$ at cell faces and the momenta $(\rho \vv)$ at the 
cell centers through the implicit Euler steps in 
\eq{eq:HalfTimePredictorFluxCorrection} and \eq{eq:TimeIntegrator}, 
respectively, we employ both cell and node centered approximations of the 
Exner pressure, respectively, see \eq{eq:ExnerPressures} and
...
\end{equation}
%
in our standard configuration. We will discuss the effects of such a 
...
The generalized forcings in \eq{eq:EulerP} are discretized in 
...
Thus, the divergence of some vector field $\vV = (U,V,W)$ utilizes
...
\item Standard fluid dynamcis tests (with minimal documentation).

\item Straka's, and rising bubble tests?
...
\item \red{Blending in this paper or reserved for the next one?}
...
\item differences relative to the compressible EULAG/FVM
  ...
  \item \red{Clear concept for achieving second order accuracy(?)}
