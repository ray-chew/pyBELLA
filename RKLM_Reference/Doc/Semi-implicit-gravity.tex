...
tuned seamlessly from solving the full compressible \benacchio{model equations}
to solving the 
...
A first order accurate time integration from the last completed time step at 
$t^n$ is sufficient for generating these half time level fluxes \benacchio{Perhaps it would
be worth explaining this as it might not be immediate for some.}. To achieve
...
\end{IEEEeqnarray}
%
Note that in this step the relation between $P$, which is being updated by
the flux divergence, and $\pi$, whose gradient is part of the momentum
forcing terms, is invoked through a linearization of the equations of state 
\eq{eq:EOSpiP}, i.e.,  
%     
...
\end{IEEEeqnarray}
%
With this linearization, the implicit step involves a single linear elliptic solve
for $\pi^{n+\half}$ as will become clear below. Optionally, an outer iteration 
...
These preliminary calculations serve to provide both the update of $P$ to the
new time level according to \eq{eq:PUpdate} in the sense of the implicit midpoint 
rule, and the advecting fluxes $(P\vv)^{n+\half}$ needed subsequently in the 
advection step of the final time stepping algorithm, see \eq{eq:AdvectionStep} 
below. 

We note for future reference that for given $(P\vv)^{n+\half}$ the update of $P$ 
is readily determined, so that the remainder of the time stepping procedure serves 
to update the vector of specific variables $\Psi$ from \eq{eq:PsiDefinition}. 
This is analogous to the role of the density update in the EULAG/FVM algorithm
summarized in section~\ref{ssec:EULAG}. 
...
by the implicit midpoint discretization of the $P$-equation in \eq{eq:PUpdate}. 
...
implicit Euler steps are $(\vu, w, \chiprime)$ only. Nevertheless, to obtain 
...
equation in perturbation form for $\piprime$ (\benacchio{is this what was the second
projection? or both the first and the second? It is not completely clear what the
...
and I think this is a potential element of novelty as well.}) from \eq{eq:EulerPiPrime} is 
...
of the discrete approximate solution consists of \benacchio{\cancel{averages of}} grid cell 
averages 
%
\begin{equation}
\Sol_{i,j,k}^n \approx 
...
cell- and timestep-centered values of the Exner pressure \benacchio{($\pi_{i,j,k}^n$)}. The latter 
corrects the cell-centered momenta and requires integrals of the Exner 
...
field to the pressure. As a consequence, \benacchio{
the implicit Euler steps control} both the advective 
fluxes $(P\vv)^{n+\half}$ at cell faces \benacchio{in 
\eq{eq:HalfTimePredictorFluxCorrection}} and the momenta $(\rho \vv)$ at the 
cell centers \benacchio{in \eq{eq:TimeIntegrator}}, 
\benacchio{thereby employing} both cell\benacchio{-centred} and node-centred approximations of the 
Exner pressure, see \eq{eq:ExnerPressures} and
...
uses the state at the previous time step, we could perhaps try that as well?} We leave testing 
...
- presumably \eq{eq:firstorder_Adv} - here instead and spare the reader a back-reference?)}, has been completed. Then, 
...
s_{i}
  & =
    & \text{Lim}
      \left(\frac{U_{i}-U_{i-1}}{\dx}, \frac{U_{i+1}-U_{i}}{\dx}\right)\,,
      \label{eq:AdvSpecificsSlopes}
\end{IEEEeqnarray}
%
where $P_{i}$ in \eq{eq:AdvSpecificsCourantNo} denotes the fourth component 
of $U_{i}$, and $\text{Lim}(a,b)$ is a slope limiting function 
...
and check the difference.}. We will discuss the effects of such a 
reduction to first order accuracy in the predictor step in 
...
The generalized \benacchio{forcing terms on the right-hand side of} \eq{eq:EulerP} are discretized in 
...
\end{equation}
%
\benacchio{(is $\tilde{X}$ not $\Thetatilde$ instead?)} and where the coefficients $(P\Theta)^*$ and $(\partial P/\partial \pi)^*$ 
...
\end{equation}
%
\benacchio{I am not sure I understand the notation, I think both the $U$ equation
and the $V$ equation should have a $1/[1+(\Delta t)^2]$ coefficient multiplying
the right-hand side instead of the $F^{-1}$.}
...
\benacchio{I tried and spelled this out:
%
 \begin{multline}\label{eq:helmholtz}
 \left(\dfrac{\partial P}{\partial\pi}\right)^*\pi'^{n+1}\\
-c_p\dt^2\left\{\dfrac{1}{1+(\dt f)^2}\left\{\left[\left(P\Theta\right)^*\pi'^{n+1}_x+\dt f\pi'^{n+1}_y\right]_x+\left[\left(P\Theta\right)^*\pi'^{n+1}_y-\dt f\pi'^{n+1}_x\right]_y\right\}\right.\\
\left.+\dfrac{1}{1+(\dt N)^2}\left[\left(P\Theta\right)^*\pi'^{n+1}_z\right]_z\right\}=R^n
 \end{multline}
%
...
\Delta t\left\{\dfrac{1}{1+(\Delta t f)^2}\left[U^n_x+V^n_y+\Delta t f(V^n_x-U^n_y)\right]
+\dfrac{1}{1+(\Delta t N)^2}\left[W^n_z+\Delta t g\left(\Thetatilde/\Thetabar\right)^n_z\right]\right\} 
\end{multline}
%
}
...
Thus, the divergence of some vector field $\vV = (U,V,W)$ \benacchio{is this
the same $U$ as above?} utilizes the discrete approximation
%
\begin{equation}
...
= 
\frac{1}{\dx} \left(\pi'_{i,j,k} + \pi'_{i+1,j,k}\right)\,,
\end{equation}
%
...
\widetilde{\left(\pi'_x\right)}_{i+\half,j,k} 
\approx 
\frac{1}{\dx} \left({\pibar}'_{i,j,k} + {\pibar}'_{i+1,j,k}\right)\,,
\end{equation}
%
where
%
\begin{IEEEeqnarray}{rCl}\label{eq:NinePointStencilII}
{\pibar}'_{i,j,k} = \frac{36}{64} \pi'_{i,j,k} 
  & + 
