...
\subsection*{January 19, 2018 (additions of January 25, 2018):}
%
\begin{itemize}
...
\item Can I invent a scheme that explicitly advects $\tilde\chi$ instead of
      full theta and thus allows me to control up-down symmetry better?\\
      (Part of my next move.)
\item Tested initial pressures with hydrostatic balance relative to background
      $\theta$ and relative to $\tilde\theta$:  The latter gives slightly 
      favorable results -- less (though anyway small) wiggles, slightly higher
      wave amplitudes. But the solves become substantially more expensive.
\end{itemize}
%

\subsection*{January 25, 2018: New scheme}
...
\item \label{it:LeadingSplitStep}
      ...
      of $\tilde\chi$, with the contribution from the background 
...
\item Advective flux correction including a correction that takes the 
      ...
      background be handled and added to $\tilde\chi$. 
\item All fluxes, in particular the density flux, should be kept from the
      split-step prediction and then fully corrected to reflect the 
      mid time level state. The density flux, in particular, has to be
      composed of the background and perturbation contributions 
      associated with $\overline{\chi}$ and~$\tilde\chi$.
...
\item Options for compressible advection:
  ...
  \item Standard procedure based on $(\rho, \rho\vv, P)$. The split into
        an additional transport scheme for $\tilde\rho = (P\tilde\chi)$
  ...
  \item Switch to formulating advection in terms of $(P\tilde\chi, P\vv, P)$. 
        In this approach, we likely don't even have to invoke an additional
        variable. 
  \item Find a way to post-correct the density flux after solving the 
        cell-centered correction equation so as to build a meaningful 
        full density flux (hopefully without having to redo the upwinding).
  \end{enumerate}
\item Options for gravity predictor
  \begin{enumerate}
  \item Keep well-balanced explicit gravity computation as is prior to this 
        development.  
  \item Evaluate gravity from $\tilde\chi$.
  \item Take gravity out of advection entirely in split-fashion, moving it
        to after advection is completed. (EULAG variant)
  \end{enumerate}
...
ToDo-List: (not done yet: \notdone; \quad done: \done)
%
\begin{itemize}
\item[\done] 
      Introduce advection of density perturbation in {\tt rhoX}-Variables.
\item[\done]
      Make advective fluxes available inside the split step advection 
      routines.
\item[\notdone]
      Access them everywhere to represent the advecting velocities.
      \begin{itemize}
      \item[\notdone] In the reconstruction.
      \item[\notdone] In the flux function.
      \end{itemize}
\item[\notdone] 
      Switch off gravity source term entirely in the advection routines. 
\item[\notdone] 
      Introduce entire gravity source term as part of the pair of projections. 
      \begin{itemize}
      \item[\notdone] apply half time step cell center gravity
      \item[\done] recompute fluxes
      \item[\notdone] correct fluxes and post-correct all advective contributions
      \item[\notdone] runs second pressure solve towards half time level
      \item[\notdone] apply resulting source term another time to get to new time level
      \end{itemize}
\item[\done] 
      Move projections in between the advection split steps.
\item[\notdone] 
      Make sure second split sequence works with advective fluxes determined
      by the first projection. 
\item[\notdone] 
      Make sure second split sequence includes vertical advection of $\overline{\chi}$. 
\item[\notdone] 
      Make sure the second split sequence does not advect $\overline{\chi}$ for
      the $\tilde\chi$ variable. 
\item[\notdone] 
      Compare results of full density advection with that of perturbation
      density / $\tilde\chi$ advection
\item[\notdone] 
      In the second half step, which quantities am I to advect? Those fully
      updated to the new time level or those strictly computed for the half 
      time level? In fact, depending on how the details are done, I can 
      recover Piotr's time integration strategy:
      \begin{enumerate}
      \item[\notdone] Euler forward advection over $\Delta t/2$
      \item[\notdone] Euler implicit pressure/gravity for the advective fluxes
                      over $\Delta t/2$ \\ (advective fluxes done)
      \item[\notdone] Euler explicit pressure/gravity for centered data 
                      from old time level over $\Delta t/2$
      \item[\notdone] Second order advection of resulting fields over $\Delta t$
                      based on previously estimated advective fluxes
      \item[\notdone] Euler implicit pressure/gravity for centered data 
                      from the result over $\Delta t/2$
      \end{enumerate}
\item[\notdone] Check influence of how advecting velocity for MUSCL-update 
      is computed from advecting fluxes in {\tt recovery\_gravity()}. Is division
      by the local {\tt Sol->rhoY[i]} OK?
\end{itemize}


\section*{Other clean-up operations}
%
\begin{itemize}
\item[\notdone] 
      Move output-related memory flipping into the output routine.
