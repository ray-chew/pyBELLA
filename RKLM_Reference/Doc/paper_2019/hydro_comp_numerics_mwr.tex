...
The first test is a diagonally moving density bump into a doubly-periodic $[0,1]\times[0,1]\,\textrm{m}^2$ domain (see full initial data at \cite{KadiogluEtAl2008}). The numerical scheme is run on a uniform grid made of $192x192$ cells with advective CFL$=0.33$ and in absence of gravity. Driven by unitary background speed the initial perturbation travels diagonally through the domain. Given the unitary setup parameters, the vortex should move back to the initial position after $T=1\,\textrm{s}$. This is confirmed by the numerical simulation, as seen in Figure \ref{fig:trav_vortex_res}. \benacchio{TBD convergence test}.
...
\centering
 ...
 \caption{Travelling vortex results on a run with $192x192$ cells, advective CFL 0.33. Initial data (top) and values after $1$ second (bottom), for the density (left) and nodal pressure (right). Contours as in \cite{Benacchio2014}, Figure 2.}
 \label{fig:trav_vortex_res}
\end{figure}

\subsection{Weakly nonlinear acoustic wave}

The second test is the evolution of a rightward acoustic perturbation in a unitary periodic one-dimensional domain. The initial data for velocity, density and pressure are:
%
\begin{equation}
 u=\dfrac{2}{\gamma-1}\tilde{c}\sin{\left(\dfrac{2\pi}{L}x\right)},\quad\rho=\rho_{ref}\left[1+\dfrac{\tilde{c}}{c_{ref}}\sin{\left(\dfrac{2\pi}{L}x\right)}\right]^{\frac{2}{\gamma-1}}, \quad p=p_{ref}\left[1+\dfrac{\tilde{c}}{c_{ref}}\sin{\left(\dfrac{2\pi}{L}x\right)}\right]^{\frac{2\gamma}{\gamma-1}}
 \label{eq:init_data_acouwave}
\end{equation}
%
The initial amplitude is tuned by varying the $\tilde{c}$ parameter. We consider a low-amplitude case with $\tilde{c}=0.01c_{ref}$, and a higher-amplitude case with $\tilde{c}=0.05c_{ref}$ and use the model . We refer to \cite{Benacchio2014} for the complete setup data for this case. A grid of 256 cells is used, the time step is $\Delta t=6.68E-05\,\textrm{s}$ and the final time is $2.67E-03\,\textrm{s}$ (40 time steps). Unlike in the other tests, a time step size within the acoustic CFL constraint is chosen in order to resolve the wave more accurately. Values of density and velocity at final time (Figure \ref{fig:acouwave}) closely mirror the ones obtained in previous work by the authors and in \cite{Vater2013}. The results display the ability of the code to model compressibility and validate the new compressible strategy both in the low-amplitude and in the higher-amplitude case, where nonlinearity effects are more pronounced.
...
%
Boundary conditions are solid walls on top and bottom boundaries and periodic elsewhere. The buoyancy-implicit model is run at a resolution $\Delta x=\Delta z=50\,\textrm{m}$ with time step chosen according to an advective CFL value of $0.96$ \benacchio{Definition of CFL value}. Driven by its negative buoyancy, the initial perturbation moves downwards, impacts the bottom boundary and travels sideways developing vortices (Figure \ref{fig:straka}). \benacchio{Insert considerations after viscosity coding, 1d profiles at different resolutions.}
...
\centering
 ...
 \caption{Potential temperature perturbation at times $t=0\, s$ (top left), $t=300\, s$ (top right), $t=600\, s$ (bottom left), $t=900\, s$ (bottom right) for the density current test case.}
 \label{fig:straka}
\end{figure}
...
\end{equation} 
%
with $H=10~\textrm{km}$, $x_c=100~\textrm{km}$, $a=5~\textrm{km}$, and there is a background horizontal flow $u=20~\textrm{m.s}^{-1}$. We consider three configurations for the horizontal extension $x_N=300\,\textrm{km},\,6000\,\textrm{km},\,48000\,\textrm{km}$, with respective final times $T=3000\,\textrm{s},\,6000\,\textrm{s},\,48000\,\textrm{s}$. The first two configurations correspond to the nonhydrostatic case and the hydrostatic case of \cite{SkamarockKlemp1994}, the third planetary-scale configuration is introduced in this paper. In all configurations, the model is run with $1200x40$ cells with the buoyancy-implicit mode and advective CFL value of 0.9. 

In the first configuration, driven by buoyancy the initial perturbations spreads out onto gravity waves driven by the underlying buoyancy stratification (Figure \ref{fig:SK94_NH}). In the second configuration, which is run with rotation (Coriolis parameter value $f=10^{-4}\,\textrm{s}^{-1}$) , a geostrophic mode is present in the centre of the domain (Figure \ref{fig:SK94_H}). In both cases, the obtained values closely resembles published results in the literature including, for the nonhydrostatic case, the buoyancy-explicit result in \cite{BenacchioEtAl2014}.
...
\centering
 ...
 \caption{Initial (above) and final (below) potential temperature perturbation for the nonhydrostatic inertia-gravity wave test from \cite{SkamarockKlemp1994}. Contour values in the range $[-0.025, 0.025]\,\textrm{K}$ with a $0.005\,\textrm{K}$ interval.}
 \label{fig:SK94_NH}
\end{figure}
...
\centering
 ...
 \caption{Initial (above) and final (below) potential temperature perturbation for the hydrostatic inertia-gravity wave test from \cite{SkamarockKlemp1994}. Contours as in Figure \ref{fig:SK94_NH}}
 \label{fig:SK94_H}
 \end{figure}
...
\centering
 ...
 \caption{Initial (above) and final (below) potential temperature perturbation for the planetary-scale inertia-gravity wave test. Contour values in the range $[-0.05, 0.05]\,\textrm{K}$ with a $0.01\,\textrm{K}$ interval.}
  \label{fig:SK94_P}
\end{figure}

As a further corroboration of the properties of the model, the hydrostatic configuration is rerun with a different value of the Coriolis parameter $f=1.03126*10^{-4}\,\textrm{s}^{-1}$, $\theta(z=0)=250\,\textrm{K}$, and no background flow as in \cite{BaldaufBrdar2013}. \benacchio{Information on the resolution to be added if we decide to rerun at very high resolution.} Results with the buoyancy-implicit model display good symmetry and compare well with the published ones (Figure \ref{fig:BB13}). The multiscale nature of the case is evident in particular the plot of the vertical velocity.
...
\centering
 ...
 \caption{Potential temperature perturbation (top), vertical velocity (middle) and horizontal velocity (bottom) at final time $T=28800\,\textrm{s}$ for the inertia-gravity wave test with rotation, parameters as in \cite{BaldaufBrdar2013}. Initial $\theta$ perturbation as in Figure \ref{fig:SK94_H}.}
  \label{fig:BB13}
\end{figure}
