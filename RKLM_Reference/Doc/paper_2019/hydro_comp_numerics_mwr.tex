...
A major hurdle towards joining the numerical scheme of \cite{BenacchioEtAl2014} with the theoretical setup of \cite{KleinBenacchio2016} is the former's time step dependency on the speed of internal gravity waves. This places a severe constraint on the applicability of the numerical method to large-scale tests. 
...
in \eq{eq:PerturbationVariables} and \eq{eq:BackgroundHydrostatics}. \benacchio{We remark that this is a fundamental feature of the present scheme; other models in atmospheric flows use the perturbation variables as prognostic quantities throughout.}
...
accurate version of our advection scheme, see section~\ref{sec:DiscretizationDetails}\ref{ssec:Advection} for details.  In the pseudo-incompressible case the discretization guarantees that $(P\vv)^n$ is divergence free, so that $P^{\#} = P^n$ and the $\apsinc$ parameter need not be explicitly noted in \eq{eq:HalfTimePredictorAdvectionII}.
...
at the half time level as needed for the advection substep. Below we summarize this implicit step in a 
temporal semi-discretization, provide
the node-based spatial discretization, and explain how the divergence-controlled momenta are used to generate divergence-controlled
...
\Thetatilde_t
  & =
    & - W\frac{d\Thetabar}{dz}
      \\
\left(\frac{\partial P}{\partial \pi}\right)^{\circ}\pi'_t
  & =
    & - U_x - V_y - W_z\,,
\end{IEEEeqnarray}
%
...
{\Thetatilde}^{n+1}
  & =
    & {\Thetatilde}^{n} - \dt\frac{d\Thetabar}{dz}\, W^{n+1}
      \\
\left(\frac{\partial P}{\partial \pi}\right)^{\circ} {\pi'}^{n+1}
  & =
    & \left(\frac{\partial P}{\partial \pi}\right)^{\circ} {\pi'}^{n} 
...
W^{n+1} 
  & =
    & \left(\frac{W + \dt\, g {\Thetatilde}/\Thetabar}{1 + (\dt\, N)^2}\right)^n
      - \dt \, \frac{c_p (P\Theta)^{\circ}}{1 + (\dt\, N)^2} \ {\pi'}^{n+1}_z \,. 
\end{IEEEeqnarray}
%
...
\end{IEEEeqnarray}
%
Analogous formulae hold for the other Cartesian directions. The flux divergence
...
of the implicit Euler step \benacchio{in \eq{eq:LinearizedNonAdvectiveImplicitEuler}}
...
calculate divergence-controlled cell-centered momenta. 
...
evolution~\eq{eq:LinearizedNonAdvectiveImplicitEulerPi}. The Exner pressure is stored on grid nodes, but so that the flux divergence on the right hand side of \eq{eq:LinearizedNonAdvectiveImplicitEulerPi} is node-centered but not cell-centered. We observe, however, that a simple node-to-cell average, 
%
...
\end{IEEEeqnarray}
%
yields a second-order accurate approximation to the cell average. We can then approximate the cell-centered divergence by the adjacent node-centered
divergences. It turns out that this is equivalent to determining the cell-face
...
as in, e.g., \citep{BellEtAl1989,AlmgrenEtAl2006,KleinTCFD2009,BenacchioEtAl2014}, and the present scheme works with the node-based Helmholtz equation 
...
The algorithm described in the previous Sections was tested on a suite of benchmarks of dry compressible dynamics at various scales. The suite draws on the set of \cite{Benacchio2014,BenacchioEtAl2014} including a cold air bubble and nonhydrostatic inertia-gravity waves, and adds to it three larger scale configurations for the inertia-gravity waves, with the aim to validate the robustness and accuracy of the new buoyancy-implicit strategy. We remark that the present paper does not focus on efficiency. While the coding framework is 3D-ready, we leave parallelization and performance on three-dimensional tests for future work. The scheme was implemented in a C$++$ environment and uses the Bi-CGSTAB linear solver \citep{Vandervorst1992} for the solution of the elliptic problems. The solver tolerance was set at $10^{-8}$ throughout. As in \cite{BenacchioEtAl2014}, we define the advective Courant number (CFL) as:
%
\begin{equation}
