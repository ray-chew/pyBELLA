...
\abstract{As a necessary step towards an ``all-scale blended multimodel'' solver for atmospheric dynamics with seamless access to \klein{compressible, soundproof, and hydrostatic} dynamics, we introduce a second-order numerical scheme for atmospheric motions at small to planetary scales. The \klein{collocated finite volume method treats the advection of mass, momentum, and mass-weighted potential temperature in conservation form while relying on Exner pressure for the pressure gradient term. It discretises the rotating compressible equations by evolving full rather than perturbation variables, and operates with time steps constrained by the advection speed only. Perturbation variables are used during a time step, but only as auxiliary quantities that enable the semi-implicit formulation. Borrowing ideas from Smolarkiewicz and Margolin (1993) (Mon.\ Wea.\ Rev., {\bfseries 121}, 1847--1859) on ``forward-in-time differencing'', the algorithm reframes the authors' previously proposed schemes} into a sequence of implicit midpoint, advection, and implicit trapezoidal steps that allows for a time integration unconstrained by the internal gravity wave speed. Compared with existing approaches, results on a range of benchmarks of nonhydrostatic and hydrostatic-scale dynamics are competitive. The \klein{test} suite includes a new planetary-scale inertia-gravity wave test that highlights the properties of the scheme, its large time step capabilities\klein{, and its capability of solving the compressible, pseudo-incompressible, and hydrostatic model equation within a uniform discretization framework.}} 
...
The method uses an explicit second-order MUSCL scheme for \klein{advection, while the pressure and momentum equations are stably integrated by solving two elliptic problems embedded in the implicit midpoint and implicit trapezoidal stages}. The scheme is validated  against two-dimensional travelling vortex tests and Cartesian benchmarks of nonhydrostatic and hydrostatic dynamics. Simulations of inertia-gravity wave tests at large scale and with rotation show competitive performance with existing approaches.\kleincomment{Shall we emphasize coarse-grid performance?} In particular, a new planetary-scale extension of the hydrostatic-scale test of \cite{SkamarockKlemp1994} showcases the large time step capabilities of the present scheme.

The paper is organized as follows. Section \ref{sec:GoverningEquations} contains the governing equations that are discretized with the methodology summarised in section \ref{sec:TimeDiscretizationSummary} and detailed in section~\ref{sec:DiscretizationDetails}. Section \ref{sec:Results} documents the performance of the code on the abovementioned tests. Results are discussed and conclusions drawn in section \ref{sec:Conclusions}. \red{An Appendix contains further algorithmic details for the interested reader. -- still needed?}
...
% The appendix contains details of the numerical scheme that are not essential at first reading and for an assessment of the results.
...
These spacial discretizations inserted into the temporal semi-discretization
of the implicit Euler step in \eq{} 
...
\begin{figure}
\centering
 \includegraphics[width=.45\columnwidth]{./TravellingVortex_192/rho/rho_snapshot0.eps}\includegraphics[width=.45\columnwidth]{./TravellingVortex_192/p2_n/p2_n_snapshot0.eps}\\
 \includegraphics[width=.45\columnwidth]{./TravellingVortex_192/rho/rho_snapshot1.eps}
 \includegraphics[width=.45\columnwidth]{./TravellingVortex_192/p2_n/p2_n_snapshot1.eps}
...
\begin{figure}
\centering
 \includegraphics[width=.48\columnwidth]{./Straka_50m/dY/dY_snapshot0.eps}
 \includegraphics[width=.48\columnwidth]{./Straka_50m/dY/dY_snapshot1.eps}\\
 \includegraphics[width=.48\columnwidth]{./Straka_50m/dY/dY_snapshot2.eps}
 \includegraphics[width=.48\columnwidth]{./Straka_50m/dY/dY_snapshot3.eps}
