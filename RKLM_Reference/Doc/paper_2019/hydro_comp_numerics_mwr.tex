...
\end{equation}
%
The initial amplitude is tuned by varying the $\tilde{c}$ parameter. We consider a low-amplitude case with $\tilde{c}=0.01c_{ref}$ and a higher-amplitude case with $\tilde{c}=0.05c_{ref}$. We refer to \cite{Benacchio2014} for the complete setup. A grid of 256 cells is used, the time step is $\Delta t=6.68\textrm{E-}05\,\textrm{s}$ and the final time is $2.67\textrm{E-}03\,\textrm{s}$ (40 time steps). Unlike in the other tests, a time step size within the acoustic CFL constraint is chosen in order to model the acoustic wave. Values of density and velocity at final time (Figure \ref{fig:acouwave}) closely mirror the ones obtained in previous work by the authors and in \cite{Vater2013}. The results display the ability of the code to model compressibility and validate the new compressible strategy both in the low-amplitude and in the higher-amplitude case, where nonlinearity effects are more pronounced.

\benacchio{Comments: need to dig out analytical solution as in \cite{Benacchio2014}, at later time (100 time steps) does not look as good as in \cite{Benacchio2014}.}
...
\benacchio{General comment, results are nondimensionalized so in order to make them comparable with the literature I hacked the colorbar and postmultiplied the values in it by $300\,K$, is it correct to do the same in all the plots in the SK case as well although the stratification looks different? What about the velocity plots in the Baldauf-Brdar case? There I hacked the colorbars by multiplying the values of both u and w by 10 (reference u); this way they look in the range of the colorbars in \cite{BaldaufBrdar2013} but as it is reverse engineering it would be good to double check.}

\begin{figure}
\centering
 \includegraphics[width=.48\columnwidth]{./Straka/dY/dY_snapshot0.eps}
 \includegraphics[width=.48\columnwidth]{./Straka/dY/dY_snapshot1.eps}\\
 \includegraphics[width=.48\columnwidth]{./Straka/dY/dY_snapshot2.eps}
 \includegraphics[width=.48\columnwidth]{./Straka/dY/dY_snapshot3.eps}
