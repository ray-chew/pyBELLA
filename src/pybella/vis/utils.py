...
class test_case(object):
    ...
    def get_filename(self, N, suffix, format="h5"):
        if self.ndim == 2:
            fn = "%s_ensemble=%i_%i_%i_%.6f_%s.%s" % (
                self.base_fn,
                N,
                self.grid_x,
                self.grid_y,
        ...
        if self.ndim == 3 or self.grid_z is not None:
            fn = "%s_ensemble=%i_%i_%i_%i_%.6f_%s.%s" % (
                self.base_fn,
                N,
                self.grid_x,
                self.grid_y,
    ...
    def py_out(pyfile, py_dataset, time):
        return pyfile[str(py_dataset)][str(py_dataset) + time][:], pyfile[
            str(py_dataset)
        ][str(py_dataset) + time].attrs.get("t")
    ...
    def get_time_series(
        ...
        if self.ndim == 3 and slc[0] == None:
            assert (
                0
            ), "3D array has no 2D slice, define argument slc=(slice(...),slice(...),slice(...))"
        ...
        for time in times:
            arr = self.get_arr(
                path, time, N, attribute, tag=tag, label_type=label_type, inner=True
            )
