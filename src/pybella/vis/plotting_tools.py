...
class plotter(object):
    ...
    def set_cax_axes(self, cax, n):
        if hasattr(self, "x_locs"):
            cax.set_xticks(self.x_locs)
        if hasattr(self, "x_axs"):
            cax.set_xticklabels(self.x_axs)
        if hasattr(self, "y_locs"):
            cax.set_yticks(self.y_locs)
        if hasattr(self, "y_axs"):
            cax.set_yticklabels(self.y_axs)
        cax.tick_params(axis="x", pad=15)
        if self.sharexlabel:
            if int(n // self.ncols) == self.nrows - 1:
                if hasattr(self, "x_label"):
                    cax.set_xlabel(self.x_label)
        else:
            if hasattr(self, "x_label"):
                cax.set_xlabel(self.x_label)
        if self.shareylabel:
            if n % self.ncols == 0:
                if hasattr(self, "y_label"):
                    cax.set_ylabel(self.y_label)
        else:
            if hasattr(self, "y_label"):
                cax.set_ylabel(self.y_label)
        if hasattr(self, "axhline"):
            cax.axhline(self.axhline, c="k", lw=0.5)
        ...
        if self.N > 1:
            ...
            for n, arr in enumerate(self.arr_lst):
                ...
                if method == "imshow" and lvl is not None:
                    ...
                    plt.colorbar(
                        im, cax=bax, ticks=lvl, extend="both"
                    )  # , format='%.3f')
                else:
                ...
                if hasattr(self, "cbar_label_coords"):
                    bax.xaxis.set_label_coords(
                        self.cbar_label_coords[0], self.cbar_label_coords[1]
                    )
                ims.append(im)
    ...
    def visualise(method, cax, arr, aspect, lvls, cmap):
        ...
        if method == "imshow":
            if lvls is None:
                im = cax.imshow(
                    arr, aspect=aspect, origin="lower", cmap=cmap, norm=norm
                )
            else:
                im = cax.imshow(
                    arr,
...
class plotter_1d(object):
    def __init__(self, ncols=3, nrows=2, figsize=(12, 12), fontsize=16):
        plt.rcParams.update({"font.size": fontsize})
        self.fig, self.ax = plt.subplots(
            ncols=ncols, nrows=nrows, sharex=False, sharey=False, figsize=figsize
        )
        self.nrows = nrows
        self.ncols = ncols
