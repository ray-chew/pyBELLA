...
def interpolation_func(ensemble, x_obs, y_obs, ud):
    ...
    x, y = np.meshgrid(x, y)
    ensemble = [
        ndimage.map_coordinates(mem, [y, x], mode="wrap", order=3) for mem in ensemble
    ]
...
def bin_func(obs, ens_mem_shape):
    obs = obs.reshape(
        ens_mem_shape[0],
        obs.shape[0] // ens_mem_shape[0],
        ens_mem_shape[1],
        obs.shape[1] // ens_mem_shape[1],
    )
...
class analysis(object):
    ...
    def analyse(self, obs, obs_covar):
        ...
        Pa = (self.no_of_members - 1.0) * np.eye(self.no_of_members) / self.rho + (
            C @ self.Y.T
        )

        Lambda, P = sp.linalg.eigh(Pa)
        Pa = P @ (np.diag(1.0 / Lambda) @ P.T)

        # This is step 6 of the algorithm.
        Wa = (
            np.sqrt(self.no_of_members - 1.0)
            * P
            @ (np.diag(np.sqrt(1.0 / Lambda)) @ P.T)
        )
    ...
    def debug(self, obs_current, suffix=""):
        ...
        plt.savefig(
            "./output_images/da_debug_%s_%i_%s"
            % (self.identifier, debug_cnt // 3, suffix),
            bbox_inches="tight",
        )
        plt.close()
        debug_cnt += 1
...
class prepare_rloc(object):
    ...
    def __init__(self, ud, elem, node, dap, N, obs_X=5, obs_Y=5):
        ...
        self.cmask, self.nmask = utils.boundary_mask(
            ud, elem, node, self.pad_X, self.pad_Y
        )
    ...
    def analyse(self, results, obs, covar, mask, N, tout):
        ...
        if self.cattr_len > 0:
            results = self.analyse_by_grid_type(
                results, obs, covar[0], mask, N, tout, "cell"
            )
        if self.nattr_len > 0:
            results = self.analyse_by_grid_type(
                results, obs, covar[1], mask, N, tout, "node"
            )
        return results
    ...
    def analyse_by_grid_type(self, results, obs, covar, mask, N, tout, grid_type):
        ...
        bc_mask, mask_n = self.get_bc_mask(
            mask_p, grid_type, Nx, Ny, obs_X, obs_Y, attr_len
        )

        # Here, the 2D arrays are split into local counterparts, say of (5x5) arrays.
        X, X_bar = self.get_state(
            state_p, mask_p, Nx, Ny, attr_len
        )  # X in R in ((Nx * Ny) x k x m) and X_bar in ((Nx * Ny) * m)
        obs_p = self.get_obs(
            obs_p, mask_p, obs_X, obs_Y, Nx, Ny, attr_len
        )  # R in ((Nx * Ny) x l)
        Y, Y_bar = self.get_state_in_obs_space(
            results, mask_p, obs_attr, obs_X, obs_Y, Nx, Ny, attr_len
        )  # Y in R in ((Nx * Ny) x k x l) and Y_bar in ((Nx * Ny) * l)
        ...
        if covar is None:
            ...
            obs_covar_current = sp.sparse.eye(
                attr_len * obs_X * obs_Y, attr_len * obs_X * obs_Y, format="csr"
            )
        else:
        ...
        logging.info("\n===================")
        logging.info(
            "To split DA problem into %i chunks at %s mb each" % (chunks, mem_size)
        )
        ...
        for chunk in range(chunks):
            analysis_by_chunk.append(
                darr.from_delayed(
                    ...
                    shape=(attr_len, self.N, chunk_size),
                    dtype=np.int64,
                )
            )
    ...
    def do_analysis(self, attr_len, covar, bc_mask, mask_n, obs_p, X, X_bar, Y, Y_bar):
        ...
        for n in range(current_chunk_size):
            ...
            forward_operator = lambda ensemble: np.array(
                [mem[~np.isnan(mem)] for mem in Y[n]]
            )
    ...
    def stack(self, results, obs, mask, obs_attr, tout):
        ...
        mask_stack = np.array(mask[list(self.da_times).index(tout)][obs_attr[0]])[
            self.i2
        ]
    ...
    def get_state(self, state, mask, Nx, Ny, attr_len):
        ...
        X = st.sliding_window_view(state, (1, 1), axis=(2, 3)).reshape(
            self.N, attr_len, Nx * Ny
        )
        X_bar = darr.from_array(X_bar)
        X_bar = st.sliding_window_view(X_bar, (1, 1), axis=(2, 3)).reshape(
            1, attr_len, Nx * Ny
        )
    ...
    def get_state_in_obs_space(
        self, state, mask, obs_attr, obs_X, obs_Y, Nx, Ny, attr_len
    ):
        """
        ...
        sios = st.sliding_window_view(sios, (obs_X, obs_Y), axis=(2, 3)).reshape(
            self.N, attr_len, Nx * Ny, obs_X, obs_Y
        )
        Y_bar = darr.from_array(Y_bar)
        Y_bar = st.sliding_window_view(Y_bar, (obs_X, obs_Y), axis=(2, 3)).reshape(
            1, attr_len, Nx * Ny, obs_X, obs_Y
        )
    ...
    def get_bc_mask(self, mask, type, Nx, Ny, obs_X, obs_Y, attr_len):
        ...
        bc_mask = st.sliding_window_view(bc_mask, (obs_X, obs_Y)).reshape(
            Nx * Ny, obs_X, obs_Y
        )

        mask = darr.from_array(mask)
        mask_n = st.sliding_window_view(mask, (obs_X, obs_Y), axis=(1, 2)).reshape(
            attr_len, Nx * Ny, obs_X, obs_Y
        )
