...
def da_interface(results, obs, obs_attributes, delta, times, tout, N, loc=0):
    ...
    tmp = np.array(
        [getattr(results[:, loc, ...][n], obs_attributes[0])[inner] for n in range(N)]
    )
    tmp = tmp[:, np.newaxis, ...]

    ensemble = np.array(
        [getattr(results[:, loc, ...][n], attributes[0])[inner] for n in range(N)]
    )
    ...
    for attr in obs_attributes[1:]:
        tmp = np.hstack(
            (
                tmp,
                np.array(
                    [getattr(results[:, loc, ...][n], attr)[inner] for n in range(N)]
                )[:, np.newaxis, ...],
            )
        )
