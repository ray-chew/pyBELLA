...
class UserData(object):
    """
    Balanced shallow-water vortex with SWE <-> lake initial blending.

    The physics setup is the ``test_swe_vortex`` case (gamma = 2 SWE
    equivalence, cyclostrophic vortex; see that module's docstring); this
    variant switches on the SWE analogue of the initial comp -> psinc ->
    comp blend: at step 0 the free surface is frozen (SWE -> rigid-lid
    lake, ``do_swe_to_lake_conv``), one lake step runs, and the lid is
    released again (``do_lake_to_swe_conv``) with the nodal pressure
    blended at ``blending_weight``. Exercises the
    ``interfaces/dynamics_blending/schemes/swe_lake.py`` path.
    """
    ...
    def __init__(self):
        ...
        self.blending_conv = "swe"
        self.blending_type = "full"
        self.blending_weight = 8.0 / 16
        ...
        self.output_base_name = "_blending_swe"
