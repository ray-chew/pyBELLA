...
def _prepare_3d_system(mem, ud, dt):
    ...
    if _jax_backend(ud):
        ...
        if pole_collapse.pole_axis_present(ud):
            ...
            # master; the post-solve scatter in do_implicit_part (backend-
            # agnostic) broadcasts the master back over the ring.
            coll = pole_collapse.get(mem.node)
            mem._pole_collapse = coll
            matvec = jax_lap3D.wrap_pole_collapse(raw, coll)
            rhs_vec = coll.gather(rhs_vec)
        else:
            mem._pole_collapse = None
            matvec = raw
        return matvec, rhs_vec
