...
def lap2D(
    ...
    for idx in range(iicxn * iicyn):
        ...
        lap[idx] = (
            -hplusx_topleft * oodx2 * ((midmid - midleft) - dp2dxdy1)
            - hplusy_topleft * oody2 * ((midmid - topmid) - dp2dxdy1)
            + hplusx_topright * oodx2 * ((midright - midmid) - dp2dxdy2)
            - hplusy_topright * oody2 * ((midmid - topmid) + dp2dxdy2)
            - hplusx_botleft * oodx2 * ((midmid - midleft) + dp2dxdy3)
...
def lap2D_gather_new(
    p, iicxn, iicyn, coeffs, dx, dy, y_rayleigh, x_wall, y_wall, diag_inv, coriolis
):
    ngnc = (iicxn) * (iicyn)
    lap = np.zeros((ngnc))
    cnt_x = 0
...
def stencil_hs(elem, node, mpv, ud, diag_inv, dt):
    ...
    return lambda p: lapHS(
        p, hplusx, hplusz, hcenter, oodx2, oodz2, periodicity, diag_inv, corrf, odx, odz
    )


@nb.jit(nopython=True, cache=True, nogil=True)
def lapHS(
    p0, hplusx, hplusz, hcenter, oodx2, oodz2, periodicity, diag_inv, corrf, odx, odz
):
...
def stencil_vs(elem, node, mpv, ud, diag_inv, dt):
    ...
    if not VS:
        ...
        hplusx = mpv.wplus[xx][proj][i2].reshape(
            -1,
        )
        hplusy = mpv.wplus[yy][proj][i2].reshape(
            -1,
        )
        hcenter = mpv.wcenter[proj][i2].reshape(
            -1,
        )
        diag_inv = diag_inv[proj][i2].reshape(
            -1,
        )

        return lambda p: lap2D(
            p,
            igx,
            igy,
            iicxn,
    ...
    if VS:
        ...
        return lambda p: lapVS(
            p, hplusx, hplusy, hcenter, oodx2, oody2, periodicity, diag_inv
        )
...
def lap2D_exner(p, iicxn, iicyn, oodx2, x_periodic, x_wall):
    ...
    for idx in range(iicxn * iicyn):
        ...
        lap[idx] = oodx2 * (left - 2.0 * mid + right)
...
def stencil_5pt(elem, node, ud):
    ...
    return lambda p: lap2D_5pt(
        p, iicxn, iicyn, oodx2, oody2, x_periodic, y_periodic, x_wall, y_wall
    )
...
def lap2D_5pt(p, iicxn, iicyn, oodx2, oody2, x_periodic, y_periodic, x_wall, y_wall):
    ...
    for idx in range(iicxn * iicyn):
        ...
        lap[idx] = oodx2 * (midleft - 2.0 * midmid + midright) + oody2 * (
            topmid - 2.0 * midmid + botmid
        )
