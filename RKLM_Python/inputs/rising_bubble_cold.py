...
def sol_init(Sol, mpv, elem, node, th, ud, seed=None):
    ...
    rc = 0.05
    delthc = -2.0
    r_cold = np.sqrt((x - xc)**2 + (y - yc)**2) / rc

    perturbationc = (delthc/300.0) * (np.cos(0.5 * np.pi * r_cold)**2)
    perturbationc[np.where(r_cold > 1.0)] = 0.0

    rho = rhoY / (ud.stratification(y) + perturbation.T + perturbationc.T)
