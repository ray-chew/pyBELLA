...
def sol_init(Sol, Sol0, mpv, bdry, elem, node, th, ud):
    ...
    p = mpv.HydroState.p0[y_idx,x_idx] * (1.0 + del0 * np.sin(wn * x))**(2.0 * th.gamm * th.gamm1inv)
    rhoY = p**(th.gamminv)
    rho = np.copy(rhoY)
    c = np.sqrt(th.gamm * p / rho)

    u = (p - mpv.HydroState.p0[x_idx, y_idx]) / (rho * c) / Ma
    u_shape = u.shape
    ...
    x = node.x[x_idx]
    p = mpv.HydroState_n.p0[y_idx,x_idx] * (1.0 + del0 * np.sin(wn * x)**(2.0 * th.gamm * th.gm1inv))
    mpv.p2_nodes[y_idx,x_idx] = (p**th.Gamma - 1.0) / ud.Msq
    ...
    set_wall_rhoYflux(bdry,Sol0,mpv,elem,ud)
