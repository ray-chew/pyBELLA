...
def sol_init(Sol, mpv, elem, node, th, ud, seed=None):
    ...
    perturb = np.exp(-((X-X.max()/4)**2/(2*(0.05E+6)**2) + (Z-Z.max()/4)**2/(2*(0.05E+6)**2)))
    perturb = np.expand_dims(perturb, 1)
    perturb = np.repeat(perturb,elem.icy,axis=1)
