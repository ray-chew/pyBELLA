    ...
    def lap2D_try(p):
        ...
        for idx in range(iicxn * iicyn):
            ...
            if x_periodic:
                if cnt_x == topx:
                    shift = 1
                    ne_topleft += (iicx - shift)
                    ne_botleft += (iicx - shift)

                if cnt_x == botx:
                    shift = 1
                    ne_topright -= (iicx - shift)
                    ne_botright -= (iicx - shift)
            ...
            if y_periodic:
                if cnt_y == topy:
                    shift = 1
                    ne_topright += ((iicy - shift) * iicx)
                    ne_topleft += ((iicy - shift) * iicx)

                if cnt_y == boty:
                    shift = 1
                    ne_botleft -= ((iicy - shift) * iicx)
                    ne_botright -= ((iicy - shift) * iicx)
            ...
            #     if cnt_x == 1:
                ...
                if cnt_y == boty:
                    ...
                    # oodx2 *= 0.5
                    # oody2 *= 0.5
                    # botleft = coeff*midleft
                    # botmid = coeff*midmid
                    # botright = coeff*midright
...
def stencil_9pt_3rd_try(rhs,elem,node,mpv,ud):
    igx = elem.igx
    igy = elem.igy
